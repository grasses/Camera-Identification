...
class BoardUtil(object):
    ...
    def vote_accuracy_curve(self, matrix, classes, mutiple_base, steps):
        (title, name, info) = self.gen_into("Voting_Accuracy", steps=steps, base=-1)
        ...
        for index in range(len(classes)):
            print matrix.T[index][index]
            plt.plot(mutiple_base, np.array(matrix.T[index][index], dtype=np.float32) / total_sum, label="{:s}".format(classes[index]))
