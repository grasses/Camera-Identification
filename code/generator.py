...
class Generator(object):
            ...
            if (not is_train) and self.with_process:
                if not os.path.exists(full_name):
                    if self.process_type == "compress":
                        self.ProcessUtil.compress_single("{:s}/{:s}".format(self.input_path, full_name))
    ...
    def model_iterator(self, is_train=False):
        model_list = self.tmp_var["model_list"]
        self.read_directory()
        for label in model_list:
            label = str(label)
            model_path = self.config["label_path"][label]
            for model in self.config["test_list"][label]:
                full_name = "{:s}/{:s}".format(model_path, model)
                file_path = "{:s}/{:s}".format(self.input_path, full_name)
                try:
                    image = misc.imread(file_path)
                    (width, height) = (image.shape[0], image.shape[1])
                    ...
                    for index in range(self.batch_size):
                        try_crop_times = 0
                        (patch_imgage, offset_x, offset_y, image_noise) = ([], 0, 0, 0)
                        while offset_x == 0:
                            try_crop_times += 1
                            if try_crop_times > self.model_conf["max_fail_times"]: exit(1)
                            (patch_imgage, offset_x, offset_y, image_noise) = self.crop(image, width, height, index)
                        yield (patch_imgage, int(label), full_name, offset_x, offset_y, image_noise)
                except Exception as e:
                    print("=> [error] iterator_read() e={:s}".format(e))
                    exit(1)
    ...
    def iterator(self, session=None):
        ...
        if type(session) == tf.Session:
            ...
            elif type(session) == type([]):
                self.tmp_var["model_list"] = session
                iterator_read = self.model_iterator
            ...
            for (image_data, image_label, image_name, offset_x, offset_y, image_noise) in iterator_read(is_train=False):
                ...
                tmp_data["image_data"][patch_count]= image_data # info: image_data = np.reshape(image_data, [config["SHAPE"][0] * config["SHAPE"][1] * config["SHAPE"][2]])
                ...
                if patch_count == self.batch_size - 1:
                    ...
                    (count, patch_count, tmp_data) = (count + 1, -1, rebuild())
